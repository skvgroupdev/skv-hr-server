...
export class EmployeesRepository {
  ...
  async create(tenantId: Types.ObjectId, dto: CreateEmployeeDto): Promise<EmployeeDocument> {
    const data: Record<string, unknown> = {
      ...
      supervisorId: dto.supervisorId ? new Types.ObjectId(dto.supervisorId) : null,
    };
    delete data.initialPassword;
    return this.employeeModel.create(data);
  }

  async findById(id: string, tenantId: Types.ObjectId): Promise<EmployeeDocument | null> {
  ...
  async update(
    ...
    if (dto.supervisorId) updateData.supervisorId = new Types.ObjectId(dto.supervisorId);
    return this.employeeModel.findOneAndUpdate({ _id: id, tenantId }, updateData, { returnDocument: 'after' }).exec();
  }
  ...
  async linkUser(id: string, tenantId: Types.ObjectId, userId: Types.ObjectId): Promise<void> {
    await this.employeeModel.findOneAndUpdate({ _id: id, tenantId }, { userId }).exec();
  }
  ...
  async findByUserIdAndTenant(
    ...
    return this.employeeModel
      .findOne({ userId, tenantId })
      .select('employeeCode firstName lastName status branchId')
      .exec();
  }

  async countByTenant(tenantId: Types.ObjectId): Promise<number> {
    return this.employeeModel
      .countDocuments({ tenantId, status: { $nin: ['RESIGNED', 'TERMINATED'] } })
      .exec();
  }

  async countActive(tenantId: Types.ObjectId): Promise<number> {
    return this.employeeModel.countDocuments({ tenantId, status: 'ACTIVE' }).exec();
  }

  async findByIds(ids: string[], tenantId: Types.ObjectId): Promise<EmployeeDocument[]> {
    return this.employeeModel
      .find({
  ...
  async findAllActive(tenantId: Types.ObjectId, branchId?: Types.ObjectId): Promise<EmployeeDocument[]> {
  ...
  async generateNextCode(tenantId: Types.ObjectId, yearMonth: string): Promise<string> {
